...
bool BitmapClass::Initialize(ID3D11Device* device, ID3D11DeviceContext* deviceContext, int screenWidth, int screenHeight, char* textureFilename, int renderX, int renderY)
{
	...
	// Initialize 함수에서는 화면 크기와 이미지가 렌더링되는 위치가 모두 저장됨
	// 이는 렌더링하는 동안 정확한 정점 위치를 생성하는 데 필요
	...
	// 그 다음 버퍼가 만들어 지고 이 비트맵 이미지의 텍스처도 로드됨
...
// 렌더링은 2D 이미지의 버퍼를 그래픽 카드에 넣음.
// 또한 입력으로 이미지가 화면에 그려질 위치를 받고 UpdateBuffers 함수가 위치를 지정하는 데 사용됨
// 이전 프레임과 위치가 바뀌었다면 동적 정점 버퍼의 정점들의 위치를 새로운 위치로 갱신함 그렇지 않으면 UpdateBuffers 함수를 그냥 지나침
...
// 2D 이미지를 그리는 데 사용할 꼭짓점 및 인덱스 버퍼를 빌드하는 데 사용되는 함수
bool BitmapClass::InitializeBuffers(ID3D11Device* device)
{
	...
	// 이전 렌더링 위치는 먼저 음수로 초기화됨
	// 이것은 이 이미지를 마지막으로 그린 위치를 찾는 중요한 변수가 됨
	// 마지막 프레임 이후 이미지 위치가 변경되지 않은 경우 동적 정점 버퍼를 수정하지 않으므로 일부 주기를 절약할 수 있음
	// Initialize the previous rendering position to negative one.
	m_prevPosX = -1;
	m_prevPosY = -1;

	// 두 개의 삼각형으로 정사각형을 만들기 때문에 꼭짓점을 6으로 설정했기 때문에 6개의 점이 필요함
	...
	// ModelClass와 비교하여 큰 차이는 다음과 같음
	// 여기서 동적 정점 버퍼를 생성하여 필요하다면 정점 버퍼 내부의 값을 수정할 수 있게 함
	// 동적이라는 속성을 지정하기 위하여 description의 Usage 변수를 D3D11_USAGE_DYNAMIC으로 하고 CPUAccessFlags를 D3D11_CPU_ACCESS_WRITE로 함
	...
	// 꼭짓점의 좌표가 변경될 수 있더라도 6개의 인덱스가 항상 동일한 6개의 꼭짓점을 가리키기 때문에 인덱스 버퍼를 동적으로 만들 필요가 없음
...
bool BitmapClass::UpdateBuffers(ID3D11DeviceContext* deviceContext)
{
	...
	// 이 이미지를 렌더링하는 위치가 변경되었는지 확인함
	// 변경되지 않은 경우 정점 버퍼는 이 프레임에 대한 변경이 필요하지 않으므로 종료됨
	// 이 검사를 통해 많은 처리 비용을 절약할 수 있음
	...
	// 이 이미지를 렌더링할 위치가 변경된 경우 다음에 이 함수를 사용할 때 새 위치를 기록합니다.
	...
	// 이미지의 4면을 계산해야 합니다.전체 설명은 자습서 맨 위에 있는 다이어그램을 참조하십시오.
	...
	// 이제 좌표가 계산되었으므로 임시 꼭짓점 배열을 만들고 새 6개의 꼭짓점으로 채웁니다.
	...
	vertices[1].position = XMFLOAT3(right * 2.0f, bottom, 0.0f);  // Bottom right.
	...
	// 이제 Map 및 memcpy 함수를 사용하여 꼭짓점 배열의 내용을 꼭짓점 버퍼에 복사합니다.
